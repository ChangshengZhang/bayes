    ...
    def UpdateTheta(self,it):
        ...
        if new_rho_sigma > self.limit:
            ...
            if new_lambda_sigma >self.lambda_sigma:
                new_sigma_sq[0] = new_sigma_sq[0]+ stats.gamma.rvs(new_lambda_sigma-self.lambda_sigma,scale= new_mu_sigma/new_lambda_sigma)
            else:
                new_sigma_sq[0] = new_sigma_sq[0]*stats.beta.rvs(new_lambda_sigma,self.lambda_sigma-new_lambda_sigma)

            # print "test:",self.rho_sigma
            # print self.lambda_sigma
            # print self.mu_sigma
            # print self.sigma_sq
            ...
            for ii in range(1,self.T):
                ...
                # print "test:"
                # print new_rho_sigma,new_lambda_sigma,new_mu_sigma,new_sigma_sq[ii-1]
                new_mean = new_rho_sigma/(1-new_rho_sigma)*new_lambda_sigma/new_mu_sigma*new_sigma_sq[ii-1]
                #print "old new mean", new_mean,old_mean
                if new_mean > old_mean:
                    new_kappa_sigma_sq[ii-1] = self.kappa_sigma_sq[ii-1]+stats.poisson.rvs(new_mean-old_mean)
                else:
                    new_kappa_sigma_sq[ii-1] = stats.binom.rvs(self.kappa_sigma_sq[ii-1],1.0*new_mean/old_mean)
                ...
                if new_lam > old_lam:
                    new_sigma_sq[ii] = new_sigma_sq[ii] + stats.gamma.rvs(new_lam-old_lam,scale = 1.0/new_gam)
                else:
            ...
            if z_star[jj]== True:
                ...
                if new_rho_beta[jj] >self.limit or new_rho >self.limit:
                    ...
                    if new_lambda > self.lambda_[jj]:
                        new_psi_star[0][counter] = new_psi_star[0][counter]+ stats.gamma.rvs(new_lambda-self.lambda_[jj],scale= new_mu/new_lambda)
                    else:
                    ...
                    for ii in range(1,self.T):
                        ...
                        if new_mean > old_mean:
                            new_kappa_star[ii-1][counter] = kappa_star[ii-1][counter]+ stats.poisson.rvs(new_mean-old_mean)
                        else:
                            new_kappa_star[ii-1][counter] = stats.binom.rvs(kappa_star[ii-1][counter],1.0*new_mean/old_mean)
                        ...
                        #print "old new:",old_lam,old_gam,new_lam,new_gam

                        new_psi_star[ii][counter] = psi_star[ii][counter]*old_gam/new_gam

                        if new_lam > old_lam:
                            new_psi_star[ii][counter] = new_psi_star[ii][counter] + stats.gamma.rvs(new_lam-old_lam,scale= 1.0/new_gam)
                        else:
                            new_psi_star[ii][counter] = new_psi_star[ii][counter]*stats.beta.rvs(new_lam,old_lam-new_lam)

                        #print new_psi_star[ii][counter]
