...
def ClacKalmanFilter(beta,phi_new_list,varphi_old,sigma_square,x,y):
    ...
    for tt in range(1,len(phi_new_list[0])):
        ...
        for kk in range(len(beta)):
            if phi_new_list[kk][tt-1]!=0:
                temp_temp_varphi = np.sqrt(phi_new_list[kk][tt]/phi_new_list[kk][tt-1])*varphi_old[kk]
            else:
...
def UpdateTheta(u_star,lambda_star,alpha_hat,alpha_hat_sigma,iterNum,eta,eta_sigma,sigma_old,k_sigma_old,beta,lambda_old,u_old,rou_old,varphi_old,xi_old,s_xi_old_list,lambda_sigma_old,u_sigma_old,rou_sigma_old,xi_sigma_old,s_xi_sigma_old,phi_old_list,k_old_list,x,y):
    ...
    temp_var_cov = np.dot(s_xi_sigma_old,S_xi_sigma)
    xi_sigma_1 = stats.multivariate_normal.rvs(mean=xi_sigma,cov=temp_var_cov,size=1)
...
##计算两个接受概率，由于伪代码原因，ap与ap_sigma求不出！！！这里跑不通！！
    ...
    ap=np.exp(min(0,p_y_phi_new_sigma_old-p_y_phi_old_sigma_old + np.log(prior_theta_new)-np.log(prior_theta_old)))
    ap_sigma= np.exp(min(0,p_y_phi_old_sigma_new-p_y_phi_old_sigma_old + np.log(prior_theta_sigma_new)-np.log(prior_theta_sigma)))
    s_xi_new=[]
    xi_new =[]
...
##更新xi_sigma以及s_xi_sigma
    xi_sigma_new=[]
    temp = stats.binom.rvs(1,ap_sigma,size=1)
    if temp[0]==1:
